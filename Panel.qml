...
Panel {
  id: root
  moduleName: "better-omarchy-bluetooth"
  ipcTarget: "better-omarchy-bluetooth"
  ...
  // Absolute path to this plugin's pairing helper. A plain relative path in a
  // Process command would be resolved against the shell's cwd, so resolve it
  // against this QML file instead (same trick as the activity-monitor plugin).
  readonly property string pairHelperPath: decodeURIComponent(
    String(Qt.resolvedUrl("bin/omarchy-bluetooth-pair")).replace("file://", ""))

  // Pairing-with-PIN dialog state. BlueZ can ask a host to enter a PIN/passkey
  // during classic pairing; Quickshell's Bluetooth API exposes no agent, and
  // the stock omarchy agent is NoInputNoOutput (cannot answer). So pairing a
  ...
  // Runs bin/omarchy-bluetooth-pair <address> [pin]. The helper owns the whole
  // sequence (power, trust, agent, pair, connect) and exits 0 only when BlueZ
  // reports the device paired, so the panel only has to report the outcome.
  Process {
    id: pairProcess
    command: []
    onExited: function(exitCode, exitStatus) {
      root.pairDialogBusy = false
  ...
  KeyboardPanel {
    ...
    PanelKeyCatcher {
      id: keyCatcher
      anchors.fill: parent
      // While the PIN dialog is up, let the TextField own the keyboard instead
      // of the panel cursor machine consuming j/k/Enter/Escape.
      blocked: root.pairDialogOpen
